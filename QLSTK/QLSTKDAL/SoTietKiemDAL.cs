...
namespace QLSTKDAL
{
    public class SoTietKiemDAL
    {
        ...
        public bool themSoTietKiem(SoTietKiemDTO stk)
        {
            ...
            query += "INSERT INTO [tblSoTietKiem] ([MaSoSTK], [MaKH], [SoTienGoiSTK], [MaLTK], [NgayMoSo], [TienLai], [SoLanDaoHan], [SoDu], [LaiSuatCamKet], [KyHanCamKet], [NgayChinhSua]) ";
            query += "VALUES (@MaSoSTK, @MaKH, @SoTienGoiSTK, @MaLTK, @NgayMoSo, @TienLai, @SoLanDaoHan, @SoDu, @LaiSuatCamKet, @KyHanCamKet,@NgayChinhSua)";
            //----------------------------
            ...
            using (SqlConnection con = new SqlConnection(ConnectionString))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    cmd.Parameters.AddWithValue("@NgayChinhSua", stk.StrNgayChinhSua);
                    //---------------------------
        ...
        {
            string query = string.Empty;
            query += "UPDATE tblSoTietKiem SET SoDu = @SoDu, SoTienGoiSTK = @SoTienGoiSTK, NgayChinhSua = @NgayChinhSua Where MaSoSTK = @MaSoSTK ";
            using (SqlConnection con = new SqlConnection(ConnectionString))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
        ...
        public List<SoTietKiemDTO> selectListSTK(string masoSTK, string maKH, string maLTK, double minSoDu, double maxSoDu, DateTime minNgay, DateTime maxNgay)
        {
            ...
            query += " WHERE ([MaSoSTK] LIKE '%"+masoSTK+"%')";
            query += " AND ([MaKH] LIKE '%"+maKH+"%')";
            query += " AND ([MaLTK] LIKE '%"+maLTK+"%')";
            query += " AND ([SoDu] >= " + minSoDu.ToString() + ")";
            if(maxSoDu > 0)
            {
               query += " AND ([SoDu] <= " + maxSoDu.ToString() + ")";
            }
            query += " AND ([NgayMoSo] >= '" + minNgay.Date.ToString() + "')";
            query += " AND ([NgayMoSo] <= '" + maxNgay.Date.ToString() + "')";
           ...
           
        {
            ...
            if (stk.IKyHanCamKet == 0) return 0;
